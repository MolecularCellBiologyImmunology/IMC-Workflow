...
def cell_distances(cutoff, data, allData, domain, clustering, lst1, lst2, save_ind, save_all):
    ...
    for ROI in ROIs:
        ...
        # Xiaofei change this part, check the output ROI_ID in R after ruuning 
        if distances.empty == True:
            print(f"No neighbours within {cutoff} pixels were identified for {data['ROI_ID']}")
            continue
...
output_path = f"{path}/Results/Spatial_output/"
#os.makedirs(output_path, exist_ok=True)


# Load the data 
# celldata = pd.read_csv(f"{path}/Data/Figures_input/250729_celldata.csv")
celldata = pd.read_csv(f"{path}/Results/Spatial_output/Community/Rphenograph20_250731_community4.csv")
# Only take cell columns of interest
# list(celldata)
celldata = celldata[['ROI_ID', 'cell_ID', 'ExpGroup', 'nb1','Location_Center_X', 'Location_Center_Y']]
...
neighbours = cell_distances(25, celldata, allData = "1", domain = "", clustering = "nb1", lst1 = [], lst2 = [],
save_ind = "HNSCC_neighbours_distance_calculation_nb1_community4.csv", save_all = "HNSCC_neighbours_distanceCalculation_allCells_nb1_community4.csv")
